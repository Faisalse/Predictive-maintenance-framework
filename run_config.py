...
import pickle
# extend pickle module for serlialization and deserliazation.....
#import dill
...
def run_single(conf, slice=None):
    ...
    train, test = dl.load_data(conf['data']['folder'], conf['data']['prefix'])
    ...
    for k, a in algorithms.items():
        ...
        eval_algorithm(train, test, k, a, evaluation,  metrics, results)
...
def run_opt_single(conf, iteration, globals):
    ...
    if 'opts' in conf['data']:
        train, test = dl.load_data(conf['data']['folder'], conf['data']['prefix'], train_eval=train_eval,
                                               )
    else:
        train, test = dl.load_data(conf['data']['folder'], conf['data']['prefix'], train_eval=train_eval)
...
def eval_algorithm(train, test, key, algorithm, eval, metrics, results):    
    ...
    results[key] = eval.evaluate_sessions(algorithm, key, metrics, test, training_time)
